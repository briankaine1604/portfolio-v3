...
const editProjectSchema = insertProjectSchema.omit({ slug: true });

export function EditProjectForm({ slug }: Props) {
  const router = useRouter();
  const inputRef = useRef<HTMLInputElement | null>(null);
  const maxTags = 12;

  const trpc = useTRPC();
  const { data } = useSuspenseQuery(trpc.project.getOne.queryOptions(slug));
  const mutation = useMutation(trpc.project.edit.mutationOptions());
      ...
      mutation.mutate(
        { slug, data: values },
        {
          onSuccess: () => {
            ...
            console.log("Current queries:", queryClient.getQueriesData({}));
